...
**User**: Agile coach or manager organizing team structure
**Outcome**: Complete visual representation of squad(s) created from simple prompt
**Context**: Primary use case - create organizational structure visualization quickly

**Acceptance Criteria**:
- User provides simple prompt: "Create 3 squads with PO, SM, 5 developers"
- Claude creates complete visualization using MCP tools
- Each squad has frame container
- Team members as sticky notes with color coding (PO=yellow, SM=green, Dev=blue)
- Reporting lines showing SM → team members
- Total creation time <5 minutes
- Result matches user's mental model

**Implementation Notes**:
- Orchestrates multiple tool calls (frames, sticky notes, connectors)
- Smart positioning algorithm to avoid overlaps
- Color conventions: Product Owner (yellow), Scrum Master (green), Developers (blue)
- Example spacing: squads 400px apart horizontally

**Source**: PROJECT_REQUIREMENTS (Primary use case example)

---

### SQUAD2: User shows reporting lines between team members

**User**: Agile coach visualizing team hierarchy
**Outcome**: Clear visual hierarchy with connectors showing reporting structure
**Context**: Understanding "who reports to whom" is critical for org clarity

**Acceptance Criteria**:
- Connectors created from team members to Scrum Master
- Connectors created from Scrum Master to Product Owner (if applicable)
- Arrow direction indicates reporting direction
- Captions optional (e.g., "reports to")
- Lines don't overlap unnecessarily

**Implementation Notes**:
- Use create_connector tool for each relationship
- Arrow end cap pointing toward manager
- Consider line color consistency
...
**User**: Agile coach mapping inter-squad collaboration
**Outcome**: Dependency lines between squads showing collaboration points
**Context**: Multiple squads often have dependencies requiring coordination

**Acceptance Criteria**:
- Connectors between different squad frames
- Different visual style than reporting lines (e.g., dashed)
- Captions indicating dependency type
- Clear visual distinction from intra-squad relationships

**Implementation Notes**:
- Different stroke style (dashed vs solid) for dependencies vs reporting
- Different colors for different dependency types
- Position dependency lines between squad boundaries
